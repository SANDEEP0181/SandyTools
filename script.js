...
function setup(){
  var templates={
    ...
    imageCompressor:'<input type="file" id="compressImage" accept="image/*"><label>Quality: <span id="qualityValue">70</span>%</label><input type="range" id="compressQuality" min="10" max="100" value="70"><button type="button" onclick="compressImageFile()">Compress Image</button><div id="compressResult" class="result">Select an image first.</div><div class="ai-actions"><button type="button" id="downloadCompressedImage" style="display:none">Download Image</button></div>',
    imageResizer:'<input type="file" id="resizeImage" accept="image/*"><input type="number" id="resizeWidth" placeholder="Width in pixels"><input type="number" id="resizeHeight" placeholder="Height in pixels"><label><input type="checkbox" id="keepRatio" checked> Keep ratio</label><button type="button" onclick="resizeImageFile()">Resize Image</button><div id="resizeResult" class="result">Select an image first.</div><div class="ai-actions"><button type="button" id="downloadResizedImage" style="display:none">Download Image</button></div>',
    qrGenerator:'<input type="text" id="qrText" placeholder="Enter text or website URL"><button type="button" onclick="generateQR()">Generate QR Code</button><div id="qrResult" class="result">Enter text or URL first.</div><div class="ai-actions"><button type="button" id="downloadQRImage" style="display:none">Download QR</button></div>',
...
window.compressImageFile=function(){imageFromFile("compressImage",function(img,f){if(!img){result("compressResult","Select an image first.");return;}var q=(Number($("compressQuality").value)||70)/100,c=document.createElement("canvas");c.width=img.naturalWidth;c.height=img.naturalHeight;c.getContext("2d").drawImage(img,0,0);c.toBlob(function(b){if(!b){result("compressResult","Compression failed.");return;}window.sandyCompressedImage=b;var db=$("downloadCompressedImage");if(db){db.style.display="inline-block";db.onclick=function(){downloadBlob(window.sandyCompressedImage,"SandyTools-Compressed.jpg");};}result("compressResult","Compression complete. Click Download Image.");},"image/jpeg",q);});};
window.resizeImageFile=function(){imageFromFile("resizeImage",function(img){var w=Number($("resizeWidth").value),h=Number($("resizeHeight").value);if(!img){result("resizeResult","Select an image first.");return;}if(!w){result("resizeResult","Enter a valid width.");return;}if($("keepRatio").checked)h=Math.round(w*img.naturalHeight/img.naturalWidth);if(!h){result("resizeResult","Enter a valid height.");return;}var c=document.createElement("canvas");c.width=w;c.height=h;c.getContext("2d").drawImage(img,0,0,w,h);c.toBlob(function(b){window.sandyResizedImage=b;var db=$("downloadResizedImage");if(db){db.style.display="inline-block";db.onclick=function(){downloadBlob(window.sandyResizedImage,"SandyTools-Resized.jpg");};}result("resizeResult","Resized to "+w+" × "+h+" px. Click Download Image.");},"image/jpeg",.92);});};
...
window.makePassportPhoto=function(){imageFromFile("passportFile",function(img){if(!img){result("passportResult","Select a photo first.");return;}var c=document.createElement("canvas"),ctx=c.getContext("2d");c.width=413;c.height=531;var s=Math.max(c.width/img.width,c.height/img.height),w=img.width*s,h=img.height*s;ctx.drawImage(img,(c.width-w)/2,(c.height-h)/2,w,h);c.toBlob(function(b){window.sandyPassportImage=b;var db=$("downloadPassportImage");if(db){db.style.display="inline-block";db.onclick=function(){downloadBlob(window.sandyPassportImage,"SandyTools-Passport-Photo.jpg");};}result("passportResult","Passport photo created. Click Download Image.");},"image/jpeg",.92);});};
